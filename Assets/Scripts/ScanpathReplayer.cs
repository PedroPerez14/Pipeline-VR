...
public class ScanpathReplayer : MonoBehaviour
{
    ...
    IEnumerator Start()
    {
        ...
        gazePositionIndicator.SetActive(loadEyeTrackingData);                     //Disable the green sphere if we're not using confidence data
    ...
    void Update()
    {
        if(enableReplay)
        {
            ...
            float t = ((float)currentReplayTimestamp - headLogTimestampCache[currentLogOnReplay][lastSampleIndex]) / (headLogTimestampCache[currentLogOnReplay][nextSampleIndex] - headLogTimestampCache[currentLogOnReplay][lastSampleIndex]);
